...
def _gate_aruco_dict():
    ...
    if _gate_dict is None:
        _gate_dict = cv2.aruco.getPredefinedDictionary(cv2.aruco.DICT_5X5_100)
    return _gate_dict
...
#                  tol=0.1, settle=1.0):
#         self.target_height = target_height
#         self.climb_kp = climb_kp  # how much to correct based on how big the error is; if isn't correcting fast enough, increase this
#         self.climb_ki = 0.01  # for how long the drone has been off; increase this, increase windup
#         self.climb_kd = 0.10  # shock absorber to prevent overshoot; increase if overshooting
#         self.max_climb_speed = max_climb_speed
#         self.tol = tol
#         self.settle = settle
#         self.int_clamp = 3.0
...
#     def pid_control(self, err, err_int, err_dot, kp, ki, kd):
...
#         err = self.target_height - height(drone)
#         self.err_int += err * dt
#         self.err_int = uav_utils.clamp(self.err_int, -self.int_clamp, self.int_clamp)

#         # pid update
#         output = self.pid_control(self.err_int, (err - self.prev_err)/dt, err, self.climb_kp, self.climb_ki, self.climb_kd)
#         self.prev_err = err

#         # motion
#         v_up = uav_utils.clamp(output, -self.max_climb_speed,
#                                self.max_climb_speed)
#         send_velocity(drone, 0, v_up, 0)
