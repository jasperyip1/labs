...
def update(drone):
    ...
    if gate is not None:
        drone.flight.stop()
        print(f"Detected gate at center ({gate.cx}, {gate.cy}) with span {gate.span} pixels")
        _done = True
    if gate is None and _timer < SEARCH_TIMEOUT:
        drone.flight.send_pcmd(0, 0, 0, SEARCH_PITCH)
    if gate is None and _timer >= SEARCH_TIMEOUT:
        drone.flight.stop()
        print("No gate detected within timeout.")
        _done = True
