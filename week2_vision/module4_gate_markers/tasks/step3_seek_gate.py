...
def update(drone):
    ...
    if gate is None:
        ...
        yaw_error = gate.cx - COL_CENTER
        yaw_cmd = uav_utils.clamp(yaw_error / COL_CENTER, -MAX_YAW, MAX_YAW)
        if abs(yaw_error) < CENTER_TOL:
            pitch_cmd = APPROACH_PITCH
        else:
            pitch_cmd = 0.0
        drone.flight.send_pcmd(pitch_cmd, 0, yaw_cmd, 0)
        if abs(yaw_error) < CENTER_TOL and gate.tag_px >= TARGET_TAG_PX:
            drone.flight.stop()
            print(f"Centered on gate with tag size {gate.tag_px} px")
            _done = True
