...
def update(drone):
    ...
    yaw_rate = KP_YAW * error
    yaw_rate = uav_utils.clamp(yaw_rate, -MAX_YAW, MAX_YAW)
    drone.flight.send_pcmd(0, 0, yaw_rate, 0)
    _hold += drone.get_delta_time() if abs(error) <= TOL else 0.0
    if _hold >= HOLD_TIME:
        print(f"Held heading {TARGET_HEADING} for {HOLD_TIME} seconds.")
        _done = True
